...
class CollectionLog(models.Model):
    ...
    collected_by_customer = models.CharField(max_length=100, blank=True, null=True)
    time_collected = models.DateTimeField(blank=True, null=True)
...
class LabreqsysEditpatientpasscode(models.Model):
    id = models.BigAutoField(primary_key=True)
    code = models.CharField(max_length=5)
    created_at = models.DateTimeField()
    patient_id = models.OneToOneField('Patient', models.DO_NOTHING)

    class Meta:
        managed = False
        db_table = 'labreqsys_editpatientpasscode'


class LabreqsysUserprofile(models.Model):
    user = models.ForeignKey(AuthUser, models.DO_NOTHING)
    role = models.CharField(max_length=12)

    class Meta:
        managed = False
        db_table = 'labreqsys_userprofile'
...
class Patient(models.Model):
    ...
    landline_num = models.CharField(max_length=10, blank=True, null=True)
...
class RequestLineItem(models.Model):
    ...
    request_status = models.CharField(max_length=11)
...
class ResultReview(models.Model):
    lab_tech = models.OneToOneField(LabTech, models.DO_NOTHING, primary_key=True)  # The composite primary key (lab_tech_id, line_item_id) found, that is not supported. The first column is selected.
...
class TemplateField(models.Model):
    ...
    field_required = models.CharField(max_length=3)
    field_value = models.CharField(max_length=255, blank=True, null=True)
